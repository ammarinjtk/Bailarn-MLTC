...
from base.word2vec import train_word2vec
from config import NN_ARCHITECTURE, BATCH_SIZE, EMBEDDING_SIZE, EPOCHS
from nn.input_data import get_data_for_model
from nn.models import get_nn_model

sys.path.append(os.path.abspath(".."))
from Tokenizer import tokenizer
Word_Embedder = __import__('Word-Embedder')


class Magpie(object):

    def __init__(self, keras_model=None, word2vec_model=None, labels=None):
        self.labels = labels
        ...
        # Word_Embedder.word2vec.Bailarn_Word2Vec()

        self.tokenizer_model = tokenizer.Bailarn_Tokenizer()
    ...
    def train(self, train_dir, vocabulary, test_dir=None, callbacks=None,
        ...
        if not self.word2vec_model:
            raise RuntimeError('word2vec model is undefined.')

        if not self.tokenizer_model:
            raise RuntimeError('tokenizer model is undefined.')
    ...
    def predict_from_file(self, filepath):
        ...
        """
        doc = Document(0, filepath, tokenizer_model=self.tokenizer_model)
    ...
    def _predict(self, doc):
        ...
        for i, w in enumerate(words):
            if w in self.word2vec_model:
                x_matrix[doc.doc_id][i] = self.word2vec_model[w].reshape(1, -1)
